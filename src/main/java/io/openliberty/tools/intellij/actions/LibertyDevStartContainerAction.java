...
public class LibertyDevStartContainerAction extends AnAction {
    ...
    public void actionPerformed(@NotNull AnActionEvent e) {
        ...
        if (project == null) {
            log.debug("Unable to start dev mode in a container, could not resolve project");
            return;
        }

        final VirtualFile file = (VirtualFile) e.getDataContext().getData(Constants.LIBERTY_BUILD_FILE);
        if (file == null) {
            log.debug("Unable to start dev mode in a container, could not resolve configuration file for  " + project.getName());
            return;
        }
        ...
        if (projectType.equals(Constants.LIBERTY_MAVEN_PROJECT)) {
            startCmd = "mvn io.openliberty.tools:liberty-maven-plugin:devc -f \"" + file.getCanonicalPath() + "\"";
        } else if (projectType.equals(Constants.LIBERTY_GRADLE_PROJECT)) {
            startCmd = "gradle libertyDevc -b=" + file.getCanonicalPath();
        }

        ShellTerminalWidget widget = LibertyProjectUtil.getTerminalWidget(project, projectName, true);
        if (widget == null) {
            log.debug("Unable to start dev mode in a container, could not get or create terminal widget for " + projectName);
            return;
        } else {
