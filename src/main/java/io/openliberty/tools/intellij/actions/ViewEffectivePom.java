...
public class ViewEffectivePom extends AnAction {
    ...
    public void actionPerformed(@NotNull AnActionEvent e) {
        ...
        if (project == null) {
            log.debug("Unable to view effective pom, could not resolve project");
            return;
        }

        final VirtualFile file = (VirtualFile) e.getDataContext().getData(Constants.LIBERTY_BUILD_FILE);
        if (file == null) {
            log.debug("Unable to view effective pom, could not resolve configuration file for  " + project.getName());
            return;
        }
        // open build file
